...
- $$ p(y \vert x) $$: **Likelihood**,
- $$ p(x \vert y) $$: **Posterior** / **Updaed Belief**,
...
- $$ p(y \vert x) $$ **is** a probabilty distribution **over** $$y$$. This is the probability of having $$y$$ **given** some information about $$x$$. Because $$y$$ must take a value anyway and given the new information the corresponding distribution should still sum up to 1. But $$ p(y \vert x) $$  is **not** a distribution over $$x$$. The same story for $$ p(x \vert y) $$ .
- The reason for the name of $$ p(x) $$ and $$ p(x \vert y) $$ as prior and posterior or initial and updated will become clear later on. But the genral mindset is that, you have a belief or intital knowledge, hence $$p(x)$$, and after getting more info, or doing experiments or collecting data you **update** your belief about $$ x $$ and reach $$ p(x \vert y) $$.
- Usually $$ p(y) $$ does not get that much attention, since there is no need for it. Sometimes it is also calledthe *normalization* factor. You could say that the denominatior in the right hand side of Eq. \(\ref{Bayes}\) is the sum of its nominator: $$ p(y) = \sum_{x} p(y \vert x) p(x)  $$ by the rules of statistic or logic or common sense. And hence the name normaliztion. You could also reach the same conclusion by summing both sides over $$x$$: $$ \sum_x p(x \vert y) = 1$$ and hence  we have  $$ \sum_x \frac{ p(y \vert x) p(x) }{p(y)} = \frac{ 1}{p(y)} \sum_x p(y \vert x) p(x) = 1 $$ which is the same as what we had earlier. 
...
Check [this pdf](paperurl: 'http://academicpages.github.io/files/paper1.pdf')
